...
void DataURIStreamTest::testWithBase64()
{
	...
	{
		DataURIStream ds(URI("data:text/vnd-example+xyz;foo=bar;base64,R0lGODdhfn5+"));
		std::ostringstream ostr;
		StreamCopier::copyStream(ds, ostr);
...
void DataURIStreamTest::testError()
{
	try
	{
		// Invalid data URI (doesn't contain comma) - must throw.
		DataURIStream ds(URI("data:x"));
	}
...
CppUnit::Test* DataURIStreamTest::suite()
{
	...
	CppUnit_addTest(pSuite, DataURIStreamTest, testError);
